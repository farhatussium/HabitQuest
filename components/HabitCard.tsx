...
const HabitCard: React.FC<HabitCardProps> = ({ habit, completions, onToggle, onDelete }) => {
  ...
  const streak = useMemo(() => {
    ...
    // Fix: Explicitly type uniqueDates as string[] to ensure the loop variable 'date' is not 'unknown'
  ...
  return (
    <div className={`group bg-white p-5 rounded-2xl shadow-sm border transition-all duration-300 ${
      isCompletedToday ? 'border-indigo-100 ring-2 ring-indigo-50 bg-indigo-50/10' : 'border-slate-100'
    }`}>
        ...
        <div className="flex items-center gap-4 flex-1">
          <button 
            ...
            className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all shadow-sm ${
              ...
              : 'bg-slate-50 text-slate-300 hover:text-indigo-400 border-2 border-slate-100'
            }`}
          >
            {isCompletedToday ? (
          ...
          <div className="flex-1">
            <h4 className={`text-lg font-bold ${isCompletedToday ? 'text-indigo-900' : 'text-slate-800'}`}>
              {habit.name}
            </h4>
